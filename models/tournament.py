...
def create_tournament_model(db):
    ...
    class Tournament(db.Model):
        ...
        match_duration = db.Column(db.Integer, default=10)  # продолжительность матча в минутах (пинг-понг)
        break_duration = db.Column(db.Integer, default=5)  # перерыв между матчами в минутах (пинг-понг)
        ...
        sets_to_win = db.Column(db.Integer, default=3)  # количество выигранных сетов для победы (пинг-понг)
        status = db.Column(db.String(20), default='регистрация')  # регистрация, активен, завершен
        created_at = db.Column(db.DateTime, default=datetime.utcnow)
        admin_id = db.Column(db.Integer, db.ForeignKey('tournament_admin.id'), nullable=True)  # Связь с админом
