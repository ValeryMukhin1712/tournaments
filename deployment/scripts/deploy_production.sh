...
###############################################################################
# Deploy Production Script для Quick Score Tournaments
# Автоматический деплой в Production окружение
# 
# Использование:
...
#   - Находиться в директории /home/deploy/quick-score
###############################################################################
...
log_info "=== Деплой Production окружения ==="
log_info "Директория: $APP_DIR"

# Шаг 1: Резервное копирование БД
...
if sudo systemctl is-active --quiet tournaments; then
    ...
    log_error "Проверьте логи: sudo journalctl -u tournaments -n 50"
    exit 1
fi

# Финальная проверка
log_info "=== Проверка работы Production ==="
sleep 2
...
if ss -tuln | grep -q ":5000"; then
    log_info "✓ Порт 5000 слушается"
else
    log_warn "⚠ Порт 5000 не слушается"
fi

# Финальная информация
log_info "=== Деплой Production завершён! ==="
echo ""
log_info "Production доступен по адресу:"
echo "  https://quickscore.sytes.net/"
echo ""
