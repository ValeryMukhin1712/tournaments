...
**Важно:** Development окружение автоматически обновляется при push в ветку `dev` на GitHub.
...
```bash
# Для разработки - работайте в ветке dev
git checkout dev
# ... внесите изменения ...
git add .
git commit -m "Описание изменений"
git push origin dev  # Автоматически задеплоится в /new_dev

# Для production - перенесите изменения из dev в main
git checkout main
git merge dev
git push origin main  # Автоматически задеплоится в /
```

**Подробная инструкция:** см. файл `SETUP_DEV_BRANCH.md`
